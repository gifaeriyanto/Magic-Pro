...
export const generateBrollDescription = async (productImages: UploadedImage[], modelImage: UploadedImage | null): Promise<string> => {
    ...
    JANGAN menulis narasi pemasaran. Langsung berikan deskripsi teknis padat dalam 2 kalimat Bahasa Indonesia agar AI eksekutor tahu aset apa yang ia proses.`;
    
    const parts: Part[] = [...productImages.map(img => toPart(img))];
    if (modelImage) parts.push(toPart(modelImage));
    parts.push({ text: "Berikan identifikasi teknis aset ini secara objektif." });
...
export const generateBrollVideoPrompt = async (image: { base64: string, mimeType: string }, productDesc: string, title: string): Promise<string> => {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const systemInstruction = `Anda adalah Sutradara AI. Analisis gambar dan buat prompt gerakan kamera (English) yang sinematik untuk generator video AI (seperti Runway atau Luma). Prompt harus fokus pada pergerakan halus seperti slow pan, zoom, atau orbital shot.`;
    
    const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: { parts: [
            { inlineData: { data: image.base64, mimeType: image.mimeType } }, 
            { text: `Product context: ${productDesc}. Scene: ${title}.` }
        ] },
        config: { systemInstruction }
    });
    return response.text?.trim() || "";
...
// =================== Magic Product Services ===================

export const generateMagicProductConcepts = async (productImage: UploadedImage, isBooster: boolean, style: string, modelImage?: UploadedImage | null): Promise<any[]> => {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
...
export const generateMagicProductImage = async (productImage: UploadedImage, prompt: string, aspectRatio: string, logoImage?: UploadedImage | null): Promise<{ imageUrl: string }> => {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
...
export const generateVideo = async (prompt: string, image: ImageData): Promise<string | undefined> => {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const base64 = image.dataUrl.split(',')[1];
    let operation = await ai.models.generateVideos({
        model: 'veo-3.1-fast-generate-preview',
        prompt,
        image: { imageBytes: base64, mimeType: image.mimeType },
...
export const generateVideoAIPrompt = async (panel: any): Promise<string> => {
    ...
    const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Create a one-liner video generation prompt for this storyboard panel: ${JSON.stringify(panel)}`
    });
    return response.text?.trim() || "";
};
