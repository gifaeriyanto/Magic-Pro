...
import { generateMagicProductConcepts, generateMagicProductImage, generateStyloImage, generateProductPhoto } from '../services/geminiService';
...
const initialState: MagicProductState = {
...
type Action = 
    | { type: 'SET_FIELD'; payload: { field: keyof MagicProductState, value: any } }
...
const reducer = (state: MagicProductState, action: Action): MagicProductState => {
    switch (action.type) {
        ...
        <div className={`relative w-full h-full group ${className}`}>
            {image ? (
                <div className="h-full w-full relative">
                    <div className="w-full h-full bg-slate-50 dark:bg-black/20 rounded-xl overflow-hidden border-2 border-violet-500/50 flex items-center justify-center p-2">
                        <img src={`data:${image.mimeType};base64,${image.base64}`} alt={image.name} className="max-w-full max-h-full object-contain drop-shadow-sm" />
                    </div>
                    <button 
                        onClick={e => { e.stopPropagation(); onRemove(); }} 
                        className="absolute -top-2 -right-2 bg-red-500 text-white p-1.5 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all hover:bg-red-600 active:scale-95"
                    >
                        <X className="w-3 h-3" />
                    </button>
                    {title === "Logo" && (
                         <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-violet-600 text-white text-[8px] px-2 py-0.5 font-black rounded-full shadow-md uppercase tracking-widest whitespace-nowrap">Watermark On</div>
                    )}
                </div>
            ) : (
                <>
                  <label 
                      htmlFor={inputId}
                      className="flex flex-col items-center justify-center w-full h-full min-h-[140px] border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-all hover:border-violet-400 group/label"
                  >
                      <div className="text-slate-500 text-center p-4">
                          <div className="w-8 h-8 mx-auto text-slate-400 group-label:text-violet-500 group-label:scale-110 transition-all">{icon}</div>
                          <p className="mt-3 text-xs font-bold uppercase tracking-tight group-label:text-violet-600">{title}</p>
...
// --- MAIN COMPONENT ---
export const ProductStudio: React.FC<MagicProductProps> = ({ onNavigate }) => {
    ...
    const setField = (field: keyof MagicProductState, value: any) => dispatch({ type: 'SET_FIELD', payload: { field, value } });
    ...
    const handleGenerate = async () => {
        ...
        try {
            if (mode === ProductMode.PRODUCT_ONLY) {
                ...
                const concepts = await generateMagicProductConcepts(productImage, isBoosterMode, style);
                ...
                for (const [i, concept] of concepts.entries()) {
                    try {
                        const res = await generateMagicProductImage(productImage, concept.prompt, aspectRatio, logoImage);
                ...
                if (isBoosterMode) {
                    ...
                    concepts = await generateMagicProductConcepts(
                        productImage, 
                        true, 
                        style, 
    ...
    const inputClasses = "w-full bg-gray-50/50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl py-2 px-3 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500 text-xs transition-all dark:text-white appearance-none cursor-pointer";
    const labelClasses = "block text-[10px] font-black uppercase text-slate-400 mb-1 ml-1 tracking-widest";

    return (
        <div className="w-full">
            <FeatureHeader 
                title="Magic Product Studio"
                ...
                tutorialLink="https://www.youtube.com/watch?v=jKc1SMnHSHc&feature=youtu.be"
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                ...
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-24 space-y-6">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-700/50">
                        
                        {/* Mode Switcher */}
                        <div className="flex w-full items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-900/50 p-1 mb-8">
                            <button 
                                onClick={() => setField('mode', ProductMode.PRODUCT_ONLY)} 
                                className={`flex-1 rounded-lg py-2 text-xs font-bold transition-all ${mode === ProductMode.PRODUCT_ONLY ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700'}`}
                            >
                                {ProductMode.PRODUCT_ONLY}
                            </button>
                            <button 
                                onClick={() => setField('mode', ProductMode.WITH_MODEL)} 
                                className={`flex-1 rounded-lg py-2 text-xs font-bold transition-all ${mode === ProductMode.WITH_MODEL ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700'}`}
                            >
                                            ...
                                            <div className="pt-4 border-t border-slate-100 dark:border-white/5">
                                                <div className="flex p-1 bg-slate-100 dark:bg-slate-900/50 rounded-xl mb-4">
                                                    <button 
                                                        ...
                                                        className={`flex-1 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all ${modelSource === 'upload' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                                    >
                                                        Upload Model
                                                    </button>
                                                    <button 
                                                        onClick={() => setField('modelSource', 'ai')}
                                                        className={`flex-1 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all ${modelSource === 'ai' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                                    >
                                                        AI Generator
                                                    </button>
                                                </div>
                                                ...
                                                {modelSource === 'upload' ? (
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3 bg-slate-50 dark:bg-black/20 p-4 rounded-2xl border border-slate-100 dark:border-white/5">
                                                        <div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {(['1:1', '16:9', '9:16'] as AspectRatio[]).map(r => (
                                            <button 
                                                ...
                                                className={`p-2.5 rounded-xl flex flex-col items-center justify-center gap-1.5 border-2 transition-all ${aspectRatio === r ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/30 dark:border-indigo-500 dark:text-indigo-300' : 'bg-white dark:bg-slate-900/50 border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:border-indigo-300'}`}
                                            >
                                    ...
                                    <div className="flex justify-between items-center mb-6">
                                        ...
                                        <div className="flex items-center gap-2 mb-6">
                                            ...
                                            <button 
                                                ...
                                                className={`w-9 h-5 rounded-full transition-all relative ${isBoosterMode ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-700'}`}
                                            >
                                                <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${isBoosterMode ? 'left-5' : 'left-1'}`}></div>
                                            </button>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-3 gap-2">
                                        {(Object.values(StyleTheme)).map(s => (
                                            <button 
                                                ...
                                                className={`py-2 px-1 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${style === s && !isBoosterMode ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:text-indigo-300' : 'bg-white dark:bg-slate-900/50 border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:border-indigo-300'} ${isBoosterMode ? 'opacity-30 cursor-not-allowed' : ''}`}
                                            >
                                                {s}
                                            </button>
                                        ))}
                ...
                {/* Right Panel: Gallery */}
                <section className="flex-1 w-full bg-white/50 dark:bg-gray-900/30 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-inner min-h-[600px]">
                    <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-100 dark:border-white/5">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Studio Gallery</h3>
                            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">
                                {results.length > 0 ? `${results.length} Photos Generated` : "Ready to Render"}
                            </p>
                        </div>
                        {isLoading && (
                            <div className="flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-900/30 rounded-full border border-indigo-100 dark:border-indigo-800/30">
                                <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
                                <span className="text-[10px] font-black uppercase tracking-widest text-indigo-700 dark:text-indigo-300">Processing</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                        {results.length === 0 && !isLoading && (
                            <div className="col-span-full h-[400px] flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 text-center">
                                ...
                                <h4 className="text-lg font-bold text-slate-600 dark:text-slate-400">Belum Ada Hasil</h4>
                                <p className="text-sm font-medium mt-1 opacity-70">Konfigurasi produk Anda di panel kiri untuk memulai.</p>
                            </div>
                        )}

                        {results.map(result => (
                            <div key={result.id} className={`relative rounded-[2rem] overflow-hidden bg-white dark:bg-black shadow-lg border border-slate-200 dark:border-white/5 transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl group ${aspectClass}`}>
                                {result.status === 'loading' && (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900 animate-pulse">
                                        ...
                                        <span className="text-[10px] font-black text-slate-400 mt-4 uppercase tracking-[0.2em]">Rendering...</span>
                                    </div>
                                )}
                                {result.status === 'error' && (
                                    ...
                                    <div className="relative w-full h-full">
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-6">
                                            <div className="flex gap-3">
                                                <button 
                                                    ...
                                                    className="flex-1 bg-white/20 hover:bg-white/40 text-white py-3 rounded-xl backdrop-blur-md transition-all border border-white/20 flex items-center justify-center gap-2"
                                                >
                                                ...
                                                <a 
                                                    ...
                                                    download={`${result.name || 'magic-product'}.png`} 
                                                    className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2"
                                                >
                                        ...
                                        {result.name && (
                                            <div className="absolute top-4 left-4 right-4 flex justify-start pointer-events-none">
                                                <span className="bg-black/40 backdrop-blur-md text-white text-[9px] font-black px-3 py-1 rounded-full border border-white/10 uppercase tracking-widest truncate max-w-full">
                                                    {result.name}
                                                </span>
                                            </div>
                                        )}
            ...
            {preview.isOpen && preview.result?.status === 'done' && (
                 <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[100] flex items-center justify-center p-4 md:p-8" onClick={() => setPreview({isOpen: false, result: null})}>
                    <motion.div 
                    ...
                    >
                        <div className="relative rounded-3xl overflow-hidden shadow-2xl bg-black border border-white/10">
                            <img src={preview.result.imageUrl} alt={preview.result.name} className="max-w-full max-h-[80vh] object-contain"/>
                        </div>
                        
                        <button onClick={() => setPreview({isOpen: false, result: null})} className="absolute -top-12 right-0 text-white hover:text-slate-300 transition-colors p-2">
                            <X className="w-8 h-8" />
                        </button>
                        ...
                        <div className="mt-8 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                            <a 
                                ...
                                download={`${preview.result.name || 'magic-product'}.png`} 
                                className="flex items-center justify-center gap-3 px-10 py-4 bg-white text-slate-900 rounded-2xl font-black text-sm hover:bg-slate-100 transition-all shadow-xl active:scale-95"
                            >
