  ...
  return (
    <div className="w-full group">
      <label
        ...
        ${isDragging 
            ? 'border-indigo-500 bg-indigo-50/50 dark:bg-indigo-900/20 scale-[1.02] shadow-xl' 
            : 'border-gray-300 dark:border-gray-600 bg-gray-50/50 dark:bg-white/5 hover:border-indigo-400 hover:bg-white dark:hover:bg-white/10'
        }`}
        ...
        {uploadedImage ? (
          <div className="w-full h-full relative group-hover:opacity-95 transition-opacity bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI4IiBoZWlnaHQ9IjgiPgo8cmVjdCB3aWR0aD0iOCIgaGVpZ2h0PSI4IiBmaWxsPSIjZmZmIi8+CjxwYXRoIGQ9IkQwIDBMOCA4Wk04IDBMMCA4WiIgc3Ryb2tlPSIjZWVlIiBzdHJva2Utd2lkdGg9IjEiLz4KPC9zdmc+')]">
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300">
                <div className="bg-white/90 dark:bg-gray-800/90 text-slate-900 dark:text-white px-5 py-2.5 rounded-full shadow-2xl font-bold text-sm flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-transform">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center space-y-4">
            <div className={`p-4 rounded-2xl transition-all duration-300 shadow-sm ring-1 ${isDragging ? 'bg-indigo-100 text-indigo-600 ring-indigo-200' : 'bg-white dark:bg-white/10 text-gray-400 dark:text-gray-500 ring-gray-100 dark:ring-white/10 group-hover:text-indigo-500 group-hover:ring-indigo-200 group-hover:scale-110 group-hover:-rotate-3'}`}>
                <UploadIcon className="h-8 w-8" />
            </div>
            <div className="space-y-1">
                <p className="font-bold text-slate-700 dark:text-slate-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{t(labelKey)}</p>
                <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">{t('uploader.fileTypes')}</p>
            </div>
          </div>
        )}
      </label>
